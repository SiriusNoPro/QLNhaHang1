...
namespace QLNhaHang.DAO
{
    public class BanNhaHangDAO
    {
        public List<BanNhaHangDTO> LayTatCa()
        {
            ...
            string query = "SELECT MaBan, TenBan, MaLoaiBan, TrangThai, SoLuongKhach FROM BanNhaHang";
        ...
        {
            string query = "INSERT INTO BanNhaHang(MaBan, TenBan, MaLoaiBan, TrangThai, SoLuongKhach) " +
                           "VALUES(@a,@b,@c,@d,@e)";
            using (var conn = DatabaseHelper.GetConnection())
            using (var cmd = new SqlCommand(query, conn))
            {
        ...
        {
            string query = "UPDATE BanNhaHang SET TenBan=@b, MaLoaiBan=@c, TrangThai=@d, SoLuongKhach=@e WHERE MaBan=@a";
            using (var conn = DatabaseHelper.GetConnection())
            using (var cmd = new SqlCommand(query, conn))
            {
                cmd.Parameters.AddWithValue("@a", obj.MaBan);
        ...
        {
            string query = "UPDATE BanNhaHang SET TrangThai=@b WHERE MaBan=@a";
            using (var conn = DatabaseHelper.GetConnection())
            using (var cmd = new SqlCommand(query, conn))
            {
                cmd.Parameters.AddWithValue("@a", maBan);
                cmd.Parameters.AddWithValue("@b", (object)trangThai ?? DBNull.Value);
                conn.Open();
                return cmd.ExecuteNonQuery() > 0;
            }
        }
